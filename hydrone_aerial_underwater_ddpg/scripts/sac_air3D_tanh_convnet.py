...
class PolicyNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, log_std_min=-20, log_std_max=2):
        ...
        self.layer2 = nn.Sequential(
            nn.Conv1d(135, self.layer_size/2, kernel_size=5, stride=2, padding=2),
            nn.ReLU(),
            # nn.MaxPool1d(kernel_size=2, stride=2)
            )
        # self.drop_out = nn.Dropout()
        self.fcn1 = nn.Linear(self.layer_size/2, self.layer_size/2)

        self.fa0 = nn.Linear(6, self.layer_size/2)
    ...
    def forward(self, state):
        ...
        if (state.shape[0] == batch_size):
            ...
            for k in range(0, batch_size):
                xc = self.layer1(state[k][0:LASER_SAMPLES].reshape((1,LASER_SAMPLES,1))) 
                xc = self.layer2(xc) 
                xc = xc.reshape(xc.size(0), -1)
                xc = self.fcn1(xc).squeeze(0)
                xs = torch.relu(self.fa0(state[k][LASER_SAMPLES:])) 
            ...
            # print(state[0][0:LASER_SAMPLES].shape)
            xc = self.layer1(state[0][0:LASER_SAMPLES].reshape((1,LASER_SAMPLES,1))) 
            xc = self.layer2(xc) 
            xc = xc.reshape(xc.size(0), -1)
            xc = self.fcn1(xc).squeeze(0)
            xs = torch.relu(self.fa0(state[0][LASER_SAMPLES:])) 
            x = torch.cat((xc,xs), dim=0)
...
class SAC(object):
    ...
    def load_models(self, episode):
        ...
        self.critic.load_state_dict(torch.load(dirPath + '/Models/' + world + '/'+str(episode)+ '_value_net.pth'))
        hard_update(self.critic_target, self.critic)
        print('***Models load***')
